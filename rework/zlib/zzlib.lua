...
function zzlib.gunzipf(filename)
  local buf = { __index = data_buffer(file.read_bytes(filename)) }

  function buf:read(num)
    num = num or 1
    ...
    for i = 1, num do
        result = result .. string.char(self:get_byte())
    end
  ...
  return inflate_gzip(infl.bitstream_init(buf));
end
