...
local function printStructures(tbl)
    num_file = num_file + 1
    while file.exists('meownatica:meownatics/' .. 'save_meownatic_' .. num_file .. '.mbp') do
        num_file = num_file + 1
    end
    ...
    arbd.write(arbd_table, 'meownatica:meownatics/' .. 'save_meownatic_' .. num_file .. '.mbp')
    meow_schem.save_to_config('save_meownatic_' .. num_file .. '.mbp')
end
